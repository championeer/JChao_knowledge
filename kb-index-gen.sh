#!/usr/bin/env python3
"""
kb-index-gen.sh — Auto-generate README.md index for the knowledge base.
Usage: ./kb-index-gen.sh [--dry-run] [KB_ROOT_PATH]
"""
import json
import os
import sys
...
# Support explicit path arg, otherwise use the directory of the symlink (not resolved target)
if len(sys.argv) >= 2 and not sys.argv[1].startswith("--"):
    KB_ROOT = Path(sys.argv[1]).resolve()
else:
    # Use the symlink's directory, not the resolved target's directory
    script_path = Path(os.path.dirname(os.path.abspath(sys.argv[0])) if sys.argv[0] else __file__)
    KB_ROOT = script_path if script_path.is_dir() else script_path.parent

...
**关键设计：** [NOW.md](https://now.md/) 是唯一允许覆写的记忆文件。其他记忆文件只追加，不覆写。
...
**设计：** [INDEX.md](https://index.md/) 是导航枢纽，指向各个子目录。Agent 启动时扫描 [INDEX.md](https://index.md/)，按需加载具体文件。
...
- [INDEX.md](https://index.md/) 包含**健康度指标**（优先级、状态、最后验证日期），Agent 扫描时能立即判断哪些知识可信
- 🔴 优先级的文件 = 核心知识，永不归档
...
[memlog.sh](https://memlog.sh/) **— 日志写入脚本**
...
**优先走 L1/L2，L3 作为兜底。** 大多数检索靠 [INDEX.md](https://index.md/) 导航就能解决，语义搜索只在不确定目标时使用。
...
[INDEX.md](https://index.md/) 不仅是导航表，还是知识库的**健康度仪表盘**。每个条目带三个维度：
...
Agent 启动时扫描 [INDEX.md](https://index.md/)，能在几秒内了解整个知识库的健康状况。
...
- 日间写入追求速度（[memlog.sh](https://memlog.sh/) < 0.01s），加 CRUD 会变成 12s（语义搜索延迟）
...
- **启动变慢：** [INDEX.md](https://index.md/) 越来越长，扫描成本增加
...
[INDEX.md](https://index.md/) 的表格结构让知识库的健康状况一目了然：
...
[NOW.md](https://now.md/) — 每次对话开始时读取，了解当前状态：
...
[AGENTS.md](https://agents.md/) — 告诉 Agent 如何操作记忆：

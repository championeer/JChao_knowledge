...
source: "https://x.com/zhixianio/status/2021384359245381982"
author:
  - "[[Unknown]]"
...
tags:
  - "clippings"
---
经过连续几天的奋战，我终于完成了在安卓上运行 OpenClaw 的工具 [botdrop.app](https://botdrop.app/)（[@botdropapp](https://x.com/@botdropapp)），也总算有时间来"填坑"了。不知道最近小伙伴们有没有跟自己的 🦞 玩出更多新花样？我觉得这股风潮已经真正席卷到了各个角落，连我身边平时对科技不感兴趣的朋友也都来问我了。

最近的一个月，我自己的大部分时间都投入在了 OpenClaw 的探索上。主要有三个方向：**日常重度使用和改造** **🦞**、**降低** **🦞** **的运行门槛**（比如用闲置安卓手机跑 🦞），以及**做垂直领域的** **🦞**（比如 [owlia.bot](https://owlia.bot/)），全虾宴了属于是。
...
每个 Agent 拥有自己独立的"家当"：工作目录（存放 [SOUL.md](https://soul.md/)、[MEMORY.md](https://memory.md/) 这些人格和记忆文件）、状态目录（存放认证信息和会话记录）、以及 API Key 等敏感信息。这些东西都是独立存储的，不会自动共享。如果你想让两个 Agent 用同一个 API Key，需要手动复制 auth-profiles.json 文件给另一个。
...
现在想象一下：Telegram 区有两个窗口，一个是 [@MainBot](https://x.com/@MainBot)，后面坐着 Owlia；另一个是 [@WorkBot](https://x.com/@WorkBot)，后面坐着 Coder（一个写代码的 agent）。WhatsApp 区只有一个窗口 [@Owlia](https://x.com/@Owlia)，后面也是 Owlia。这就是多 Agent 配置的基本画面。
...
这段配置的意思是：Owlia 默认用 Sonnet，但在 [#dev](https://x.com/search?q=%23dev&src=hashtag_click) 频道里用 Opus。agents.list 里的 model 是默认值，bindings 里的 model 是针对特定路由的覆盖。
...
3. “[#dev](https://x.com/search?q=%23dev&src=hashtag_click) 频道的消息给 Coder”（peer 级别）

当你在 Office 服务器的 [#dev](https://x.com/search?q=%23dev&src=hashtag_click) 频道发消息时，三条规则都能匹配，但系统会选最具体的那条：[#dev](https://x.com/search?q=%23dev&src=hashtag_click) 频道（peer）比服务器（guildId）具体，服务器比整个 Discord（channel）具体，所以最终走 Coder。
...
Coder 负责 [#dev](https://x.com/search?q=%23dev&src=hashtag_click) 频道，其他所有消息由 Owlia 处理（因为 Owlia 设了 default: true）。每个 Agent 有自己的 workspace，里面可以放不同的 [SOUL.md](https://soul.md/) 来定义人格。identity 字段会自动用于群聊的 mention patterns，比如在群里 [@Owlia](https://x.com/@Owlia) 或 [@Coder](https://x.com/@Coder) 就会触发对应的 Agent 回复。

这里有个小技巧：虽然每个 Agent 有自己的 workspace 和 session 目录，但它们在文件系统层面并不是完全隔离的（除非开了沙箱）。当一个 Agent 的 session 出问题时（比如工具调用被截断导致一直报错），你**可以让另一个 Agent 帮忙修复**——直接告诉 Coder “帮我看看 Owlia 的 session 文件是不是格式乱了”，Coder 就能读取 ~/.openclaw/agents/main/sessions/ 下的文件，找到问题并修复。
...
所以如果你想让两个 Agent 协作，我建议这几种方案：**用 sessions\_send 直接跨 Session 通信，干净利落**，也不会打扰频道里的其他人；或者让一个 Agent 主导，另一个被动响应特定关键词；又或者用不同 Channel 隔离，需要时再手动 @ 就好。

如果要用 sessions\_send 实现跨 Agent 通信，需要在配置里显式开启：
...
这个配置允许 main 和 coder 两个 Agent 互相发消息。默认情况下 sessions\_send 只能在同一个 Agent 内使用（比如从主 session 发消息给 cron session），开启 agentToAgent 后才能跨 Agent 通信。
...
deny 的优先级高于 allow。你还可以用 group:\* 语法批量配置，比如 group:fs 代表 read、write、edit、apply\_patch 这一类文件系统工具，group:runtime 代表 exec、bash、process 这一类运行时工具。

工具限制是"软性"的——Agent 仍然在主机上运行，理论上可以访问所有文件。如果你需要更严格的隔离，比如 Agent 要对外服务（放在 Telegram 群里让陌生人用），就需要**沙箱模式**了。
...
- workspaceAccess: "ro" 让它只读访问自己的 workspace（能读 [SOUL.md](https://soul.md/) 等配置，但不能改）
...
最后聊一个我一直想实现的场景：能不能进入某个 Discord 频道时，自动注入特定的人格设定？比如在 [#writer](https://x.com/search?q=%23writer&src=hashtag_click) 频道里，我希望 Agent 自动变成"作家模式"，不需要每次都手动下指令。这样就可以进一步减少我用多个独立 Agent 的需求了。
...
然后用 openclaw hooks enable writer-channel-intro 启用。这样每次进入 [#writer](https://x.com/search?q=%23writer&src=hashtag_click) 频道的 thread，Agent 就会自动获得写作人格的加成。当然了，测试代码里做的事情是让它每次回我都发出呐喊：

![图像](https://pbs.twimg.com/media/HA1fmFZaAAEGVsU?format=png&name=large)
...
- **owlia-lite** 🦉 — Owlia 的"省电模式"，用 Sonnet。专门负责 Discord 里几个不太重要的频道，比如 [#heartbeat](https://x.com/search?q=%23heartbeat&src=hashtag_click)（状态播报）和 [#digests](https://x.com/search?q=%23digests&src=hashtag_click)（每日摘要）。
...
想更深入研究的朋友，推荐看官方文档的[Multi-Agent](https://docs.openclaw.ai/concepts/multi-agent)和[Sandboxing](https://docs.openclaw.ai/tools/multi-agent-sandbox-tools) 章节。
...
- [Agent 训练师入门指南：Clawdbot 配置与踩坑](https://x.com/zhixianio/status/2015330732756042216?s=20)
- [Agent 训练师安全课：Clawdbot 七步自检指南](https://x.com/zhixianio/status/2015661002273280099?s=20)
- [Agent 训练师案例分享：用 Clawdbot 打造专属写作工作流](https://x.com/zhixianio/status/2016445458575483084?s=20)
- [Agent 训练师进阶指南：用 Discord 打造高效 OpenClaw 协作系统](https://x.com/zhixianio/status/2018595121084994002?s=20)

English versions on [zhixian.io](https://zhixian.io/)
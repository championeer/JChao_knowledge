...
tags:
  - "clippings"
---
> 本文档详细拆解 OpenClaw Agent 发送给 LLM 的完整 System Prompt 的组成结构**版本：** v2.1 **更新时间：** 2026-03-05

## 整体架构图

![图像](https://pbs.twimg.com/media/HCooLsLboAA09HL?format=png&name=large)
...
## Layer 1: OpenClaw Framework Core（框架核心层）第一层：OpenClaw 框架核心
...
![图像](https://pbs.twimg.com/media/HCooDLkawAEl6ID?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCooX0ZawAMKTnZ?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCooutMaQAAJ0Jp?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCoozNCbwAAyGi0?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCoo5q5awAAtcZA?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCopA9sawAQjtdA?format=png&name=large)

实际示例

\## Runtime Runtime: agent=thinktank | host=黄宗宁的MacBook Air | repo=/Users/huangzongning/.openclaw/workspace-thinktank | os=Darwin 25.2.0 (arm64) | node=v25.5.0 | model=xiaowang886/claude-opus-4-6-thinking | default\_model=xiaowang886/claude-opus-4-6-thinking | shell=zsh | channel=discord | capabilities=none | thinking=off## Runtime Runtime: agent=thinktank | host=黄宗宁的 MacBook Air | repo=/Users/huangzongning/.openclaw/workspace-thinktank | os=Darwin 25.2.0 (arm64) | node=v25.5.0 | model=xiaowang886/claude-opus-4-6-thinking | default\_model=xiaowang886/claude-opus-4-6-thinking | shell=zsh | channel=discord | capabilities=none | thinking=off
...
![图像](https://pbs.twimg.com/media/HCopI0aasAAaNZ2?format=png&name=large)
...
**触发位置：** bootstrap-hooks.ts 的 applyBootstrapHookOverrides()**触发位置：** bootstrap-hooks.ts 的 applyBootstrapHookOverrides()
...
registerInternalHook("agent:bootstrap", (event) => { const context = event.context as AgentBootstrapHookContext; // 完全控制 bootstrapFiles 数组 context.bootstrapFiles = \[ { path: "CUSTOM.md", content: "自定义内容" } \]; });registerInternalHook("agent:bootstrap", (event) => { const context = event.context as AgentBootstrapHookContext; // 完全控制 bootstrapFiles 数组 context.bootstrapFiles = \[ { path: "CUSTOM.md", content: "自定义内容" } \]; });

2\. bootstrap-extra-files Hook（Bundled Hook）

**触发位置：** hooks/bundled/bootstrap-extra-files/handler.ts**触发位置：** hooks/bundled/bootstrap-extra-files/handler.ts
...
{ "hooks": { "bootstrap-extra-files": { "enabled": true, "paths": \["extra/\*.md", "docs/CONTEXT.md"\] } } }{ "hooks": { "bootstrap-extra-files": { "enabled": true, "paths": \["extra/\*.md", "docs/CONTEXT.md"\] } } }
...
**触发位置：** attempt.ts 的 runBeforePromptBuild()**触发位置：** attempt.ts 的 runBeforePromptBuild()
...
{ prompt: string; // 用户输入 messages: unknown\[\]; // Session 消息历史 }{ prompt: string; // 用户输入 messages: unknown\[\]; // Session 消息历史 }
...
{ "agents": { "defaults": { "bootstrapMaxChars": 20000, "bootstrapTotalMaxChars": 150000 } } }{ "agents": { "defaults": { "bootstrapMaxChars": 20000, "bootstrapTotalMaxChars": 150000 } } }
...
{ "hooks": { "bootstrap-extra-files": { "enabled": true, "paths": \["docs/API.md", "docs/ARCHITECTURE.md"\] } } }{ "hooks": { "bootstrap-extra-files": { "enabled": true, "paths": \["docs/API.md", "docs/ARCHITECTURE.md"\] } } }

**场景 2：我想根据任务类型动态加载文件**

推荐方案：自定义 agent:bootstrap Hook

registerInternalHook("agent:bootstrap", (event) => { const context = event.context as AgentBootstrapHookContext; const sessionKey = context.sessionKey; // 根据 session 类型加载不同文件 if (sessionKey.includes("coding")) { context.bootstrapFiles.push({ path: "CODING\_GUIDELINES.md", content: fs.readFileSync("...").toString() }); } });registerInternalHook("agent:bootstrap", (event) => { const context = event.context as AgentBootstrapHookContext; const sessionKey = context.sessionKey; // 根据 session 类型加载不同文件 if (sessionKey.includes("coding")) { context.bootstrapFiles.push({ path: "CODING\_GUIDELINES.md", content: fs.readFileSync("...").toString() }); } });

**场景 3：我想注入实时上下文（如当前时间）**

推荐方案：before\_prompt\_build Hook

on("before\_prompt\_build", (event, ctx) => { return { prependContext: \`当前时间：${new Date().toISOString()}\` }; });on("before\_prompt\_build", (event, ctx) => { return { prependContext: \`当前时间：${new Date().toISOString()}\` }; });
...
![图像](https://pbs.twimg.com/media/HCopRSBagAAHjUJ?format=png&name=large)

## 完整 System Prompt 组装流程

![图像](https://pbs.twimg.com/media/HCop3KcawAkWOuO?format=png&name=large)

![图像](https://pbs.twimg.com/media/HCop8nHbwAAo1cQ?format=png&name=large)

![图像](https://pbs.twimg.com/media/HCoqEeuawAMYh2e?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCoq4AUaEAAsLE1?format=png&name=large)
...
![图像](https://pbs.twimg.com/media/HCoq9LMakAALrnV?format=png&name=large)

**总结**

OpenClaw 的 System Prompt 不是一个单一的文件，而是 **9 层架构**的精心编排：
